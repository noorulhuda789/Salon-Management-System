...
namespace SalonManagmentSystem.UI.ServicesUi
{
    public partial class updateServiceProducts_uc : Form
    {
        ...
        private void addProd_btn_Click(object sender, EventArgs e)
        {
            ...
            string query = "SELECT id, name FROM Product WHERE name = @name AND isdeleted = 12";
        ...
        private void PopulateGV()
        {
            ...
            // Fetch product names based on their IDs
            string query = "SELECT Product.id, Product.name " +
                           "FROM ServiceProducts " +
                           "LEFT JOIN Product ON Product.id = ServiceProducts.productId " +
                           "WHERE ServiceProducts.serviceId = @serviceId";
            ...
            foreach (DataRow row in result.Rows)
            {
                ...
                DataGridViewRow dataGridViewRow = new DataGridViewRow();
                dataGridViewRow.Cells.Add(new DataGridViewTextBoxCell { Value = productId }); // Add to Product ID column
                dataGridViewRow.Cells.Add(new DataGridViewTextBoxCell { Value = productName }); // Add to Product Name column
